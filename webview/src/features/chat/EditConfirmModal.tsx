...
interface EditConfirmModalProps {
  /** What the edit would clear, or null when closed. Null *is* the closed
...
export function EditConfirmModal({
  ...
  // Escape is `Overlay`'s now. Enter and Shift+Enter stay here, still on
  // `window` and still deferred by a frame — see below.
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <Overlay
      ...
      // This dialog has never travelled — it only scales — and the sweep is not
      // allowed to add properties. Preset timing, curve and scale amplitude;
      // the `y` is dropped.
      panelMotion={{
