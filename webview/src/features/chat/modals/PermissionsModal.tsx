  ...
  return (
    ...
    >
      ...
      <div className={s.body}>
        ...
        <p className={s.note}>
          Granted from an approval card. They apply in every project you open,
          and they never cover a destructive or network call — those always ask.
        </p>
        ...
        <p className={s.note}>
          Rules the Claude CLI applies from its own settings files. LUNO shows
          them; it does not decide between them, and it cannot revoke one it did
          not make — open the file to change it.
        </p>

        {rules.length === 0 ? (
          <div className={s.empty}>
            No rules in any settings file. Nothing outside LUNO is allowing or
            denying a tool.
          </div>
        ) : (
          <ul className={s.list}>
        ...
        {unreadable.map((u) => (
          <p key={u.file} className={s.warn}>
            ...
            <span>
              The {u.source} settings at <code>{u.file}</code> could not be read
              — {u.reason}. Rules in it are not listed above, but the CLI may
              still be applying them.
            </span>
          </p>
        ))}
        ...
        {cannotRead.length > 0 && (
          <p className={s.warn}>
            ...
            <span>
              LUNO cannot read {cannotRead.join(", ")}. If your organisation
              sets policy there, it is in force and not shown here.
            </span>
          </p>
        )}
      </div>
