...
/**
 * Names of servers the user authorized through Claude Code's own `/mcp`, as
 * the last `claude mcp list` reported them.
 *
 * Exported so the CLI bridge can pre-allow their tools without reaching into
 * the cache variable — it is written here, by `refreshClaudeCodeStatus`, and
 * a second module reading it directly is how it would drift.
 */
export function cliConnectedServerNames(): string[] {
  return (cliStatusCache?.servers ?? [])
