...
- **The protocol.** Enumerate the message union in `webview/src/lib/rpc.ts` by
  ...
  A reader wants to know the shape, not all sixty names.
...
- **The design system.** `design/tokens` → `themes/*` → modules; `design/motion.ts`;
  `design/icons.tsx`; `design/primitives/*`.
- **Sizes.** Name the files over ~700 lines. `panel.ts` is the god-file and the
  map should say so plainly rather than let each newcomer rediscover it.
